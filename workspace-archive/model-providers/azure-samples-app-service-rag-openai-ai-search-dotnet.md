---
title: Azure-Samples/app-service-rag-openai-ai-search-dotnet
archive_category: model-providers
source_category: ai/rag
source_url: https://github.com/Azure-Samples/app-service-rag-openai-ai-search-dotnet
resource_id: github:azure-samples/app-service-rag-openai-ai-search-dotnet
local_vault_path: ai/rag/azure-samples-app-service-rag-openai-ai-search-dotnet.md
quality_score: 70
archive_score: 77
archive_tier: useful
resource_kind: benchmark
importance: medium
tags:
- github-repo
- html
- openai
- rag
selection_reason:
- Strong keyword match
- Active open-source repository
---

# Azure-Samples/app-service-rag-openai-ai-search-dotnet

## Why This Is In The Archive

- Matched archive category: `Model Providers`
- Quality score: 70 | Archive score: 77 (useful)
- Resource kind: benchmark
- Selection reasons:
  - Strong keyword match
  - Active open-source repository

## Summary

- **Hybrid Search Integration**: Implements hybrid search combining vector, keyword, and semantic ranking via Azure AI Search for document retrieval in a Blazor Server app.
- **Azure OpenAI RAG Pipeline**: Demonstrates Retrieval Augmented Generation (RAG) using Azure OpenAI for chat-based document interaction with managed identity security.
- **One-Command Deployment**: Supports Azure Developer CLI-based deployment and includes Blazor UI components for interactive document querying.

## Use Cases

- API key selection
- Cost/capability profiling
- Provider fallback routing

## Source

Original source URL: https://github.com/Azure-Samples/app-service-rag-openai-ai-search-dotnet

## Local Vault File

Path: [azure-samples-app-service-rag-openai-ai-search-dotnet.md](../../ai/rag/azure-samples-app-service-rag-openai-ai-search-dotnet.md)
