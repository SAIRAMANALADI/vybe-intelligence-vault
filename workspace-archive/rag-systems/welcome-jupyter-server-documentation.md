---
title: Welcome! — Jupyter Server documentation
archive_category: rag-systems
source_category: ai/rag
source_url: https://jupyter-server.readthedocs.io/
resource_id: blog:welcome-jupyter-server-documentation
local_vault_path: ai/rag/welcome-jupyter-server-documentation.md
quality_score: 70
archive_score: 72
archive_tier: useful
resource_kind: framework
importance: medium
tags:
- hackernews
- producthunt
- rag
- reddit
- web-crawled
- workflows
selection_reason:
- Strong keyword match
- Useful for project building
---

# Welcome! — Jupyter Server documentation

## Why This Is In The Archive

- Matched archive category: `RAG Systems`
- Quality score: 70 | Archive score: 72 (useful)
- Resource kind: framework
- Selection reasons:
  - Strong keyword match
  - Useful for project building

## Summary

- **Core Backend Replacement**: Jupyter Server replaces the Tornado Web Server in Jupyter Notebook, providing core services, APIs, and REST endpoints for Jupyter web applications.

- **Extension Framework**: Enables creation of applications/services via extensions (e.g., JupyterLab, Resource Usage, Scheduler) leveraging Jupyter Server’s framework and services.

- **Multi-Persona Documentation**: Separates technical documentation for users, operators, developers, and contributors, covering instal

## Use Cases

- Knowledge querying
- Document summarization
- Hybrid retrieval

## Source

Original source URL: https://jupyter-server.readthedocs.io/

## Local Vault File

Path: [welcome-jupyter-server-documentation.md](../../ai/rag/welcome-jupyter-server-documentation.md)
