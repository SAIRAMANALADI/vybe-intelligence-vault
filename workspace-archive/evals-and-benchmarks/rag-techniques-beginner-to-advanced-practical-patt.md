...
- **RAG Architecture**: Combines dense/sparse retrieval (FAISS, BM25) with LLM generation (e.g., Flan-T5) to ground responses in retrieved context, reducing hallucinations and enabling dynamic knowledge updates.

- **Core Components**: Dense embeddings (sentence-transformers), vector stores (FAISS), rerankers (cross-encoders), and hybrid retrieval (BM25 + vector search) for precision/recall optimization.

- **Advanced Patterns**: Multi-hop retrieval, GraphRAG (knowledge graphs), context compress
