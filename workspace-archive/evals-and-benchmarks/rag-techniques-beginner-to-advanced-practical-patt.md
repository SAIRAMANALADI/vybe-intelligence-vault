...
- **Minimal RAG Pipeline**: Uses `sentence-transformers` for embeddings, FAISS for vector search, and `transformers` (e.g., Flan-T5) for generation; supports optional BM25 for lexical retrieval.
- **Core Components**: Dense retrieval (embeddings + vector search), sparse retrieval (BM25/TF-IDF), rerankers (e.g., monoT5), and hybrid retrieval (combining sparse/dense results).
- **Advanced Techniques**: Multi-hop retrieval, GraphRAG (knowledge graphs), multi-vector retrievers, context compression,
