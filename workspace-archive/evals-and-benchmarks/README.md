# Evals and Benchmarks

Evaluation tools and benchmarks for LLMs, RAG systems, agents, prompts, hallucinations, and SWE tasks.

## Why It Matters

Rigorous evaluations protect against regressions in model behavior, prompt drift, and agent logic.

## Top Matched Resources

- [OpenAI · GitHub](https://github.com/openai) (framework - ESSENTIAL Tier)
- [microsoft/TREC-2019-Deep-Learning](https://github.com/microsoft/TREC-2019-Deep-Learning) (framework - ESSENTIAL Tier)
- [GoogleCloudPlatform/agent-starter-pack](https://github.com/GoogleCloudPlatform/agent-starter-pack) (template - ESSENTIAL Tier)
- [GraphRAG-Bench/GraphRAG-Benchmark](https://github.com/GraphRAG-Bench/GraphRAG-Benchmark) (framework - ESSENTIAL Tier)
- [weaviate/Verba](https://github.com/weaviate/Verba) (benchmark - ESSENTIAL Tier)
- [PacktPublishing/LLM-Engineers-Handbook](https://github.com/PacktPublishing/LLM-Engineers-Handbook) (framework - ESSENTIAL Tier)
- [Pinecone · GitHub](https://github.com/pinecone-io) (framework - ESSENTIAL Tier)
- [streamlit/example-app-langchain-rag](https://github.com/streamlit/example-app-langchain-rag) (framework - ESSENTIAL Tier)

## Best GitHub Repositories

https://github.com/tylerwince/promptfoo (Promptfoo)
https://github.com/confident-ai/deepeval (DeepEval)
https://github.com/explodinggradients/ragas (Ragas)
- [OpenAI · GitHub](https://github.com/openai)
- [microsoft/TREC-2019-Deep-Learning](https://github.com/microsoft/TREC-2019-Deep-Learning)
- [GoogleCloudPlatform/agent-starter-pack](https://github.com/GoogleCloudPlatform/agent-starter-pack)
- [GraphRAG-Bench/GraphRAG-Benchmark](https://github.com/GraphRAG-Bench/GraphRAG-Benchmark)
- [weaviate/Verba](https://github.com/weaviate/Verba)

## Best Learning Resources

- Evaluating LLM outputs using LLM-as-a-judge patterns
- Benchmarking RAG pipelines for faithfulness and answer relevance

## Project Ideas

- Write a pipeline testing prompt inputs against a test suite in Promptfoo
- Set up automated evaluation checks in a local CI workflow using DeepEval

## Related Skills

`LLM Evaluation`, `Regression Testing`

---

*Last updated: 2026-08-09 19:07 IST*
