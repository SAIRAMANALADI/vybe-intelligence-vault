---
title: What's Haystack Enterprise Platform? | Haystack Enterprise Platform Documentation
archive_category: evals-and-benchmarks
source_category: ai/rag
source_url: https://docs.cloud.deepset.ai/docs/getting-started?__hstc=17958374.34708e2a09562f4972500c174140941c.1781253286645.1782744530064.1782887335051.6&__hssc=17958374.15.1782887335051&__hsfp=f0d584f5cf03caae536faced7333acfe
resource_id: blog:what-s-haystack-enterprise-platform-haystack-enter
local_vault_path: ai/rag/what-s-haystack-enterprise-platform-haystack-enter.md
quality_score: 70
archive_score: 72
archive_tier: useful
resource_kind: framework
importance: medium
tags:
- agents
- models
- rag
- scripts
- web-crawled
- workflows
selection_reason:
- Strong keyword match
- Useful for project building
---

# What's Haystack Enterprise Platform? | Haystack Enterprise Platform Documentation

## Why This Is In The Archive

- Matched archive category: `Evals and Benchmarks`
- Quality score: 70 | Archive score: 72 (useful)
- Resource kind: framework
- Selection reasons:
  - Strong keyword match
  - Useful for project building

## Summary

- **Data Processing Pipeline**: Supports automated ingestion, OCR, and preprocessing of multi-format documents (PDFs, Office, images) via built-in converters and document stores for structured retrieval.

- **Modular AI Agent Framework**: Enables construction of RAG pipelines and multi-step AI agents using interchangeable components (retrievers, rankers, generators) with visual Pipeline Builder and model-agnostic LLM/embedding provider integration.

- **Full-Lifecycle Management**: Provides REST

## Use Cases

- Model performance auditing
- Prompt regression tracking
- Hallucination testing

## Source

Original source URL: https://docs.cloud.deepset.ai/docs/getting-started?__hstc=17958374.34708e2a09562f4972500c174140941c.1781253286645.1782744530064.1782887335051.6&__hssc=17958374.15.1782887335051&__hsfp=f0d584f5cf03caae536faced7333acfe

## Local Vault File

Path: [what-s-haystack-enterprise-platform-haystack-enter.md](../../ai/rag/what-s-haystack-enterprise-platform-haystack-enter.md)
