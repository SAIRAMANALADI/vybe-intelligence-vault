---
title: LangSmith Observability - Docs by LangChain
archive_category: evals-and-benchmarks
source_category: ai/rag
source_url: https://docs.langchain.com/langsmith/home
resource_id: blog:langsmith-observability-docs-by-langchain
local_vault_path: ai/rag/langsmith-observability-docs-by-langchain.md
quality_score: 70
archive_score: 74
archive_tier: useful
resource_kind: framework
importance: medium
tags:
- agents
- anthropic
- benchmark
- openai
- rag
- scripts
- web-crawled
- workflows
selection_reason:
- Strong keyword match
- Useful for project building
---

# LangSmith Observability - Docs by LangChain

## Why This Is In The Archive

- Matched archive category: `Evals and Benchmarks`
- Quality score: 70 | Archive score: 74 (useful)
- Resource kind: framework
- Selection reasons:
  - Strong keyword match
  - Useful for project building

## Summary

- **LangSmith Observability** provides end-to-end monitoring of LLM applications, including trace-level debugging, production metrics, and performance dashboards across frameworks like OpenAI, Anthropic, and CrewAI.
- **Core features** include automated failure detection via **LangSmith Engine**, real-time trace filtering/exporting, user feedback collection, and automated workflows via webhooks and evaluations.
- **Deployment options** include cloud, hybrid, or self-hosted setups, with all tiers

## Use Cases

- Model performance auditing
- Prompt regression tracking
- Hallucination testing

## Source

Original source URL: https://docs.langchain.com/langsmith/home

## Local Vault File

Path: [langsmith-observability-docs-by-langchain.md](../../ai/rag/langsmith-observability-docs-by-langchain.md)
