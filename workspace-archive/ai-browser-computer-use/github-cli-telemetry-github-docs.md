...
- GitHub CLI collects pseudonymous telemetry to improve product development, prioritizing features based on real-world usage patterns (e.g., subcommand adoption, flag usage).
- Telemetry can be reviewed in logging mode (`GH_TELEMETRY=log` or `gh config set telemetry log`) to inspect JSON payloads before actual transmission, including dimensions like `device_id`, `os`, and `command_invocation`.
- Opt-out is supported via environment variables (`GH_TELEMETRY=false` or `DO_NOT_TRACK=true`) or CLI c
