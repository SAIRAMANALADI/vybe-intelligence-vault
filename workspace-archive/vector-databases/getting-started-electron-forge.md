...
- **Packaging limitations**: Forge requires `node_modules` on disk and uses a naive module resolution algorithm; Yarn ≥2 must use `node-linker: node-modules`, and pnpm requires `node-linker=hoisted` in `.npmrc` to a
