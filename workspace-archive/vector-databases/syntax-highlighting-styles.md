...
- Default style is configurable in `hugo.yaml`/`hugo.toml`/`hugo.json` under `markup.highlight.style` (e.g., `monokai`).
- External CSS generation is possible via `hugo gen chromastyles` when `noClasses=false` is set in configuration.
