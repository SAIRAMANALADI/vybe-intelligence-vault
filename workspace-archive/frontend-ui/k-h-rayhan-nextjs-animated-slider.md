---
title: K-H-Rayhan/nextjs-animated-slider
archive_category: frontend-ui
source_category: web-development/framer-motion
source_url: https://github.com/K-H-Rayhan/nextjs-animated-slider
local_vault_path: web-development/framer-motion/k-h-rayhan-nextjs-animated-slider.md
tags:
- animation
- frontend_ui
resource_id: github:k-h-rayhan/nextjs-animated-slider
status: active
last_checked: '2026-06-12T15:13:16.093710Z'
health_score: 82
change_history:
- timestamp: '2026-06-12T10:40:18.782936+00:00'
  event_type: discovered
  description: Resource first seen and stored in the vault.
- timestamp: '2026-06-12T15:13:16.093710Z'
  event_type: metadata_update
  description: Forks updated from None to 86.
- timestamp: '2026-06-12T15:13:16.093710Z'
  event_type: metadata_update
  description: Archived updated from None to False.
- timestamp: '2026-06-12T15:13:16.093710Z'
  event_type: metadata_update
  description: License updated from None to MIT License.
stars: 376
forks: 86
archived: false
license: MIT License
---




# K-H-Rayhan/nextjs-animated-slider

## Why This Is In The Archive

- Matched source category: `web-development/framer-motion`
- Tags: animation, frontend_ui
- Archive category: Frontend UI

## Summary

Stunning animated slider website built using Next.js, Framer Motion, and Tailwind CSS.

## Use Cases

- UI component design
- Landing page building
- Dashboard creation

## Source

Original source: https://github.com/K-H-Rayhan/nextjs-animated-slider

## Local Vault File

Path: web-development\framer-motion\k-h-rayhan-nextjs-animated-slider-8ac2b8.md
